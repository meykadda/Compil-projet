...
void exitScope() {
 ...
 int isLogical(const char *expr) {
    if (strcmp(expr, "true") == 0 || strcmp(expr, "false") == 0) {
        return 1; // Logical constants
    }
    if (strchr(expr, '<') || strchr(expr, '>') || strstr(expr, "==") || strstr(expr, "!=") || strstr(expr, "&&") || strstr(expr, "||")) {
        return 1; // Logical/comparison operators
    }
  return 0 ; 
}
int isFloat(char *val) {
    // Vérification si la chaîne contient un point décimal, ce qui indique un float
